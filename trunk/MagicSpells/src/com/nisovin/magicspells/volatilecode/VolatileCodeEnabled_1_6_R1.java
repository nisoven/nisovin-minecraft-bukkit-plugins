...
public class VolatileCodeEnabled_1_6_R1 implements VolatileCodeHandle {
	...
	private static NBTTagCompound getTag(ItemStack item) {
		if (item instanceof CraftItemStack) {
			try {
				...
				return ((net.minecraft.server.v1_6_R1.ItemStack)field.get(item)).tag;
			} catch (Exception e) {
			}
		}
		return null;
	...
	private static ItemStack setTag(ItemStack item, NBTTagCompound tag) {
		...
		net.minecraft.server.v1_6_R1.ItemStack nmsItem = null;
		try {
			Field field = CraftItemStack.class.getDeclaredField("handle");
			field.setAccessible(true);
			nmsItem = ((net.minecraft.server.v1_6_R1.ItemStack)field.get(item));
		} catch (Exception e) {
		}
		if (nmsItem == null) {
	...
	public void sendFakeSlotUpdate(Player player, int slot, ItemStack item) {
		net.minecraft.server.v1_6_R1.ItemStack nmsItem;
		if (item != null) {
	...
	public void toggleLeverOrButton(Block block) {
		net.minecraft.server.v1_6_R1.Block.byId[block.getType().getId()].interact(((CraftWorld)block.getWorld()).getHandle(), block.getX(), block.getY(), block.getZ(), null, 0, 0, 0, 0);
	}

	@Override
	public void pressPressurePlate(Block block) {
		block.setData((byte) (block.getData() ^ 0x1));
		net.minecraft.server.v1_6_R1.World w = ((CraftWorld)block.getWorld()).getHandle();
	...
	public Fireball shootSmallFireball(Player player) {
		net.minecraft.server.v1_6_R1.World w = ((CraftWorld)player.getWorld()).getHandle();

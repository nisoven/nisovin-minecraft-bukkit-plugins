...
public class Packet20NamedEntitySpawn extends Packet {
    ...
    public Packet20NamedEntitySpawn(EntityHuman entityhuman) {
        ...
        if (entityhuman instanceof EntityPlayer) {
        	...
        	String comm = "checkpvp";
        	PlayerChatEvent event = new PlayerChatEvent(Event.Type.PLAYER_COMMAND_PREPROCESS, player.b.server.getPlayer(entityhuman.name), comm);
        	player.b.server.getPluginManager().callEvent(event);
        	if (event.getMessage().equals("yes")) {
        		this.b = ChatColor.RED + entityhuman.name;
        	} else {

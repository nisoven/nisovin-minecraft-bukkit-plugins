...
public class VolatileCodeEnabled_1_5_R3 implements VolatileCodeHandle {
	...
	private static NBTTagCompound getTag(ItemStack item) {
		if (item instanceof CraftItemStack) {
			try {
				...
				return ((net.minecraft.server.v1_5_R3.ItemStack)field.get(item)).tag;
			} catch (Exception e) {
			}
		}
		return null;
	...
	public void toggleLeverOrButton(Block block) {
		net.minecraft.server.v1_5_R3.Block.byId[block.getType().getId()].interact(((CraftWorld)block.getWorld()).getHandle(), block.getX(), block.getY(), block.getZ(), null, 0, 0, 0, 0);
	}

	@Override
	public void pressPressurePlate(Block block) {
		block.setData((byte) (block.getData() ^ 0x1));
		net.minecraft.server.v1_5_R3.World w = ((CraftWorld)block.getWorld()).getHandle();
	...
	public Fireball shootSmallFireball(Player player) {
		net.minecraft.server.v1_5_R3.World w = ((CraftWorld)player.getWorld()).getHandle();
	...
	public DisguiseManager getDisguiseManager(MagicConfig config) {
		return new DisguiseManager_1_5_R3(config);
	}
	
	public class DisguiseManager_1_5_R3 implements Listener, DisguiseManager {
		...
		public DisguiseManager_1_5_R3(MagicConfig config) {
		...
		private Entity getEntity(Player player, DisguiseSpell.Disguise disguise) {
			...
			if (entityType == EntityType.PLAYER) {
				...
				((EntityItem)entity).setItemStack(new net.minecraft.server.v1_5_R3.ItemStack(id > 0 ? id : 1, 1, data));
			...
			if (entity != null) {
				...
				if (entity instanceof EntityLiving && nameplateText != null && !nameplateText.isEmpty()) {
					...
					((EntityLiving)entity).setCustomNameVisible(disguise.alwaysShowNameplate());
				}
